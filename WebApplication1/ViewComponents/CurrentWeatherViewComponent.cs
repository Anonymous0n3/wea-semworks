...
namespace WebApplication1.ViewComponents
{
    public class CurrentWeatherViewComponent : ViewComponent
    {
        ...
        public CurrentWeatherViewComponent(IConfiguration configuration, IHttpClientFactory httpClientFactory)
        {
        ...
        public async Task<IViewComponentResult> InvokeAsync(string? location = null)
        {
            ...
            location ??= "auto:ip"; // default podle IP
            ...
            try
            {
                ...
                // Nastavení AirQualityLevel
                vm.AirQualityLevel = vm.AirQualityIndex switch
                {
                    1 => "Good",
                    2 => "Moderate",
                ...
                foreach (var day in forecast.EnumerateArray())
                {
                    ...
                    {
                        Date = DateTime.TryParse(day.GetProperty("date").GetString(), out var d) ? d : DateTime.MinValue,
            ...
            {
                // Pokud se cokoliv pokazí, vrať prázdný model
                return View(new WeatherViewModel { LocationName = "N/A" });
            }
        }
    }
